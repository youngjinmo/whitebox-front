...
export default {
  ...
  methods: {
    ...
    async verifyResetLink(username, verificationCode) {
      try {
        const response = await axios.patch(`${API_URL}/user/reset-password?username=${username}&verificationCode=${verificationCode}`);
        if (response.status === 200) {
          this.showModal('비밀번호 초기화 성공', '비밀번호 초기화가 완료되었습니다.<br>초기화된 비밀번호를 이메일로 보내드렸습니다.<br>5초 뒤 로그인 페이지로 이동합니다.');
